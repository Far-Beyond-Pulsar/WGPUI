...
Phases 0–6.3 are merged to `2.0`. Write-ups: `docs/phase-0-results.md`
through `docs/phase-6.3-results.md`. Five primitive kinds now render
byte-exact against legacy (`Quad`, `GlyphRun`, `PolySprite`, `Shadow`,
`Underline`) — but Phase 6.3 surfaced the most consequential gap yet:
**nothing in `2.0` actually emits a `Shadow` or `Underline`, because
`Div`'s style-to-primitive emission never advanced past Phase 2's minimal
proof-of-concept.** Every pipeline built so far can draw its primitive
byte-exact; almost none of them are reachable from real styled content yet.
That's now Phase 6.6, and it's arguably the single largest lever left for
example parity, since nearly every example styles a `div()`. The concrete
next steps, in order:

1. **6.6 (Div's real style emission)** — background/border/corner-radius as
   `Quad`, `box-shadow` as `Shadow`, `StyledText` highlight runs as
   `Underline`. This is what turns five working-but-mostly-unreachable
   pipelines into ones real examples actually exercise.
2. 6.4 (paths/backdrop-blur) — and don't trust its shader stub's "moved
   as-is" comment without checking; that exact claim has been false three
   times running (mono_sprites, shadows, underlines).
3. **6.5 (the animation driver)** — real per-frame interpolation over style
   deltas, driven by `request_animation_frame` (real since Phase 6), under
   the frozen `.with_animation()`/`AnimationExt` API. Closes `animation`,
   likely `smooth_scrolling`/`karaoke_*`, and GIF frame advancement in one
   pass.
4. Decide Phase 6.1's fate for real: run the fused-dispatch follow-up spike
   sketched in its rescoped row (§8), or drop the phase outright. Lower
   priority than 6.2-6.6 — a performance question, not a parity blocker.
5. Before Phase 8: address the cross-kind occlusion gap Phase 6.3 found (a
   `Quad` occluder cannot cull a `Shadow`/other-kind primitive today, since
   culling dispatches per kind — §9), and re-run Phases 0, 3, 4, 4.5, and
   6's benchmarks/window behavior on at least one non-NVIDIA/non-Windows
   configuration — every number and window-present claim so far is one
   machine, one driver, one backend, one OS.
6. Then §8's remaining phases in order: devtools extraction (7), the legacy
   alias crate + full parity run (8, §3.7/§7's sharpened bar).
7. Carried-forward, not urgent: `PrimitiveStore::reflow`'s O(n²) bulk-build
   ...
   sub-pixel aliasing quirk, Phase 5.6's disclosed narrowness, Phase 6's
   reasoned-not-observed stale-clip fix, image scaling's nearest-neighbor
   gap, and Phase 6.3's two deliberately-preserved legacy quirks (zero-blur
   NaN, squared underline alpha) (§9, all measured or named, none yet
   mitigated because none yet need to be) — address each when a real need
   arrives. `gpui-ce`'s own legacy test binary still hasn't been confirmed

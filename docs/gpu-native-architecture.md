...
exists today, until Phase 8's cutover.

**What replaces it is not assumed to be a thin re-export** — that was this
document's original guess, and it's being corrected here rather than
discovered wrong at cutover. A straight re-export only works if `2.0`'s
public types are structurally identical to `gpui-ce`'s own — same field
layouts, same trait shapes, callable the same way. Nothing in this plan
guarantees that, and several phases have already produced types that are
*behaviorally* equivalent without being *structurally* identical (`FramePlan`
losing `Clone`/`PartialEq` in Phase 2 is one disclosed example of exactly
this kind of drift). The honest plan is therefore a **legacy alias crate**:
a real adapter layer at the root (`src/`, or the crate `gpui-ce` compiles
to) that presents the *exact* old public API — same type names, same method
signatures, same trait bounds — implemented in terms of `2.0`'s internals,
remapping rather than re-exporting wherever the shapes diverge. This is
strictly more work than a re-export would have been, and it's the correct
scope, not a fallback to feel bad about: it's what "zero source changes
except the crate name in the import" (§7) actually requires when the
backend underneath has been rebuilt from the ground up, which was always
the point of this whole document. Whether any given type needs a real
adapter or can in fact re-export directly is decided per-type at cutover,
not assumed either way in advance — the file map for this crate is written
then, once that's known, not guessed at here.
...
- **Tile size is a real tuning knob with a real tradeoff**, the same shape
  as §5.0's write-granularity tradeoff — and Phase 4.5 actually measured it
  (128–1024px) rather than picking from compositor folklore. 256px won
  (11.3× vs. refill, 2.8% overlay share), landing where the ~256–512px
  common-compositor-practice guess above expected it to. But the *reason*
  small tiles lose turned out to be different from the guess: this
  paragraph originally predicted per-tile draw-call/bookkeeping overhead as
  the failure mode at small sizes; measurement found the actual binding
  constraint is **overlay overflow** — at 128px, ordinary content is larger
  than a tile, so 21.8% of the scene falls out of the grid entirely onto the
  unbuffered overlay (§4.3's own resolved multi-tile rule below).
  Draw-call overhead is real too, just not what limited the small end here.
- **Content spanning multiple tiles needs a rule** — resolved in Phase 4.5
  by actually measuring the obvious answer first: clipping isn't expressible
  for this crate's primitives (a `Quad` has no content mask, and geometric
  clipping deforms rounded corners/borders), so the fallback was "anything
  spanning tiles lives on the unbuffered overlay" — the same pattern SFD §2
  proposes for hover-resolved content. That rule **failed its own gate**
  when measured against a real scene: 73% of primitives ended up on the
  overlay, because content up to one tile wide routinely straddles a tile
  boundary depending on where it happens to sit, which is most placements,
  not an edge case. Shipped instead: a primitive no larger than one tile
  anchors to whichever tile holds its top-left corner, overhang and all;
  only content genuinely too large for one tile reaches the overlay. This is
  disclosed here rather than presented as if it were the obvious design from
  the start, because it wasn't — the "reuse the overlay pattern" instinct
  was the first thing tried, and it was wrong until measured.
...
**Status, honestly, not aspirationally**: Phase 5 gave `Img` and
`StyledText` real `diff_key` implementations — the invariant's actual
correctness-and-performance half — but neither got `estimated_size`. The
invariant as stated above is not yet fully discharged for either element;
it's `diff_key`-complete, not invariant-complete. Recorded here rather than
silently left for a future phase to discover the gap in.

---

## 7. Frontend contract, made testable

"The public API doesn't change" is enforced, not just stated, and the bar
is exact: **every example in this repository's `[[example]]` list —
everything under `examples/learn/`, `examples/bench/`, `examples/legacy/`,
plus the three top-level ones (`karaoke_text`, `karaoke_app`,
`text_gradients`) — runs unmodified against `2.0`, with the single
allowed source change across the entire set being the crate name in the
`Cargo.toml` dependency line and any `use` statement naming it.** Not most
of them, not "the ones that don't hit a deferred gap" — all of them. This
is the literal test of §3.7's legacy alias crate: if an example needs a
source change beyond the crate name to keep compiling, something in that
crate isn't actually presenting the old API, and the gap gets fixed there,
not by touching the example.

- Every example builds and runs against `wgpui-core` once it's wired in as
  an alternate backend — not "mostly," not "with the caveats listed here."
  A CI job builds every example against both backends.
...
| **1** | `wgpui-core::patch` — the patch-list protocol (§2): insert/update/remove for primitives, layout inputs, hitboxes, dispatch nodes. `wgpui-core::scene` — persistent per-layer slabs (R-N Pillar III's concept, 2.0's mechanics), accepting patches instead of full-range rewrites. **Ambient reconciliation (§4.0) ships here, window-wide, with no `.boundary()` involved at all** — `diff_key`/`InstanceKey` reconciliation and Taffy node reuse apply to every element in the tree by construction, not fenced to any subtree. **`.uncached()` (§4.2) ships in this same phase, not later** — shipping the default without its escape hatch would leave high-frequency-update UIs (the game-engine-editor workloads this crate targets) with no way to opt out of reconciliation bookkeeping that provably never pays off for them. No compute yet; CPU-computed draw ranges, same as today, just through the new protocol. **Executed** — branch `wgpui-2.0/phase-1-patch-protocol` (pushed to origin, not merged, no PR yet), off `2.0` after Phase 0. Two passes: the first left a commit that didn't compile (missing inter-crate dependency, an uncompiled 1,075-line scene module, and a 5-line stub sitting exactly where the round-trip gate needed to live — caught and fixed by verifying rather than trusting the report, same discipline this document tries to model); the second made `Scene` the real assembly point, wrote `patch/apply.rs` in full, and added the two gate tests (3, a strengthened 2) that had no coverage at all. All four gates pass with real named tests, `cargo check --workspace`/`cargo test -p wgpui-core`/`-p wgpui-layout` pass (128 + 6 tests), clippy is clean on both crates with three real fixes (not suppressions), and the diff touches nothing under `src/` or the root `Cargo.toml` — confirmed by `git diff`, not assumed. Two honest gaps, not gate failures: (a) `gpui-ce`'s own legacy test binary did not finish in 11+ minutes of CPU-bound running during verification — unmodified by this branch per the diff, so not a regression, but unconfirmed as passing; a human should let it finish once. (b) **Nothing yet produces a `Description`/`ScenePatch` from a real element** — the reconciler and patch protocol are tested end-to-end against synthetic inputs, not *driven* by anything, because deciding what primitives an element emits needs the element vocabulary §3.4 puts in `wgpui-widgets`, which doesn't exist yet. This isn't a missed gate (none asked for it) — it's the seam the next phase that touches real elements lands on, named here so it isn't rediscovered as a surprise. Full write-up: `docs/phase-1-results.md` on that branch. | A round-trip test: apply a patch sequence, read back the resident buffer, matches an equivalent full-rebuild reference exactly — **met**, via an independent oracle scene compared record-for-record (the first version derived the "rebuild reference" from the patched scene's own output, which could only prove the encoder deterministic, not correct — caught during verification, not shipped). **Separately, and just as load-bearing**: a plain, unboundaried three-level-deep div that renders identically to last frame keeps the same layout-node identity and skips its equivalent of `prepaint`/`paint` — with zero `.boundary()`, zero `.id()`, zero API touched anywhere in the test — **met**, and now mechanically checks the "zero API touched" clause instead of only asserting the identity claim. `wgpui-core` has no `prepaint`/`paint` of its own (those take `Window`/`App`, §3 puts them elsewhere); "skipped" here means the reconciler's `FramePlan` marks the node `Reused`. **A third, symmetric check**: a `.uncached()` subtree allocates no `ElementInstance` and its children's state survives across frames identically to a reconciled subtree's — **met**; had zero coverage before the second pass. `use_state`/focus don't exist yet, so this is checked structurally (`state.rs` and `instance.rs` do not import each other) plus behaviorally against a live reconciled control, not a hardcoded constant. **A fourth check, per §5.0**: changing one primitive's value inside a large layer issues one upload entry sized to that primitive's stride, not the layer's full range — **met** as headless data (an `UploadPlan` entry list; no real `wgpu::Buffer`/`write_buffer` exists yet, correctly deferred to `wgpui-wgpu`). Building 10,000 quads by appends was measured to upload 2.6× the layer's final size from size-class relocations — §5.0's disclosed non-O(1) insert/remove case, working as documented, and not what the gate measures (the gate measures one steady-state update, not a bulk build). |
| **2** | `.boundary()` (§4.1) implemented as a pure compositing/buffering policy on top of Phase 1's already-ambient reconciliation: independent GPU texture retention, auto positional identity for the boundary root, and the `Reason::Scroll` signal (§5.4) from day one — not retrofitted. Old `.layer()`/`.layer_keyed()` etc. keep working unchanged in the legacy backend; `.boundary()` only exists in `wgpui-core`. **`WgpuSurface` gets real (positional) identity + a trivial `(bounds, style, surface_id)` `diff_key` here too (§5.5, Gap 1)** — it depends on the same positional-identity work this phase already builds, and unblocks Phase 4's compositing unification. **Executed** — merged to `2.0` (PRs #152/#153 plus a wrap-up commit). Emission (the `FramePlan`→`ScenePatch` arrow Phase 1 left undone) was closed here rather than deferred further, since `.boundary()` is untestable without something real to composite. Verification caught two real problems before merge, not after: evicted boundaries never called `Scene::remove_layer` (zero call sites existed for it) and leaked a `Layer` entry forever — fixed by having eviction park released `LayerId`s and free them at the *start* of the next frame, since a patch can still name a layer released in the same call that produced it; and one of the three gate-supporting tests silently no-op'd its first case (built identical to the control, then patched at `index == 0` inside a loop) — fixed with a per-case `assert_ne!` against the control so a future no-op edit fails loudly instead of passing quietly. `cargo check --workspace` clean (72-warning `gpui-ce` baseline, unchanged, 5 pre-existing `E0133`), 183 tests passing across the three touched crates, clippy clean from a cold build with zero suppressions. `docs/phase-2-results.md` has the full account, including that `FramePlan` lost `Clone`/`PartialEq` as a side effect of the emission design — a real narrowing of a Phase 1 public type, disclosed rather than glossed over. | `.boundary()` with zero policy arguments reaches R-N's fast path (transform-only recomposite on scroll) on a plain `overflow_y_scroll` div with no other API touched — the exact test SFD Pass A's gate specifies, now true by default rather than by opt-in — **met**, and strengthened to assert consequences (empty patch, zero bytes, zero upload calls) rather than only the internal decision. Additionally: removing `.boundary()` from that same test case degrades the scroll case to a per-tick recomposite (no independent texture) but does **not** reintroduce full rebuild — confirming boundary and reconciliation are actually decoupled, not just documented as such — **met**, via two live windows compared side by side on `FrameStats` and layout-node identity, the strongest of the three gates. **`WgpuSurface` check**: an unmoved, unresized `wgpu_surface()` element skips `request_layout`/`prepaint`/`paint` across frames exactly like a reconciled `div` would — **met** against a live plain-element control, proving the pattern generically; the real `WgpuSurface` element itself is still a stub, correctly deferred to when `wgpui-wgpu` integration lands. `Retention::Texture` is a real decision with no real texture behind it yet — that's Phase 4's job, named here rather than assumed done. |
| **3** | GPU compute ordering + occlusion (§5.1, §5.2) over Phase 1's slabs. `WGPUI_OCCLUSION=validate`-equivalent differential harness ported and run against the compute path. **Executed** — merged to `2.0`. The first phase to touch a live `wgpu::Device`. An interruption left real algorithm work mid-edit (restructuring the ordering shader's iteration count); the continuation verified it rather than trusting it, found it hadn't been benchmarked yet, ran it, caught a real regression the rework introduced (serializing to ~24 workgroups on a 24-SM GPU) and fixed it with two further measured changes. Adapter: RTX 4060 Laptop GPU, Vulkan, driver 561.03 — same single machine as Phase 0; breadth caveat unimproved. `cargo check` clean, 265 tests passing, clippy clean from a cold build with zero suppressions (one fix mattered: `!(a > b)` is not `a <= b` under NaN, rewritten through `partial_cmp` rather than "simplified" into a behavior change). `docs/phase-3-results.md` has the full account. | Culled/unculled scenes match exactly over a scripted UI walk (R-N §8.5's bar) — **met**: both the CPU and compute arms of the differential harness pass, the compute arm verified through an actual pixel comparison via the GPU's own draw order, not just an order-array diff. Spike numbers from Phase 0 reproduced on the real pipeline — **met, with the honest magnitude disclosed rather than the flattering one**: two realistic scenes built through `wgpui-core`'s actual `Scene`/`Layer`/`PrimitiveStore`. Scene A (99,054 primitives, normal zoom) measured 84.2× — but this is mostly *not* GPU brilliance: the CPU `BoundsTree` degrades to ~25µs/insert on content spread this widely (worse than Phase 0's own ~7.4µs baseline, and Phase 0 predicted exactly this degradation shape), so the ratio overstates the GPU's own contribution. Scene B (97,254 primitives, a 577-deep dependency chain) measured a much more modest 1.41× — **this is the number to plan against**, not the 84×. Occlusion specifically *loses* on Scene A by 1.30×: only 3% of primitives are visible, so the CPU early-exits on the other 97% while the GPU pays upload+dispatch for all of it regardless — a real, disclosed limitation, not a hidden one. |
| **4** | Indirect draw-arg generation + `multi_draw_indirect` (§5.3), with the CPU-readback fallback for drivers/WASM that can't take it. **`WgpuSurface`/`SurfaceRegistry` compositing unification (§5.5, Gap 2)**: the *consuming* half of `SurfaceRegistry`'s composite path is folded into the same indirect-draw entry mechanism `.boundary()`'s texture-retained layers use; `SurfaceRegistry`'s producer-side triple-buffer, atomic generation tracking, and `gpu_submit_lock` cross-thread synchronization are untouched — nothing about how the external render thread paces itself changes. **Executed** — merged to `2.0`. `SurfaceRegistry`'s "moved, not rebuilt" claim was verified by diffing directly against the legacy file, not by inspection: exactly four hunks, all documented; `gpu_submit_lock`/`submit_guard` confirmed untouched by an empty `git diff -- src/`. Verification found five real things, not zero: a missing `Cargo.lock` entry that broke any `--locked` build; `QuadDrawPlan` rebuilt every frame, directly contradicting the O(layer-slots) claim gate 1 exists to prove (now cached, with a build counter asserting it in the steady-state test); two stale doc comments; and a corrected understanding of the fallback's cost model — it tracks queued GPU work, not slot count, confirmed by re-benchmarking both axes independently. **One correction to this row itself**: the gate text below claimed `WgpuSurfaceHandle` already had concurrency tests (`submit_guard`, `has_unconsumed_frame` backpressure) for the unification to "pass unmodified against" — verification found `src/elements/wgpu_surface.rs` has no tests at all. That claim was wrong when written; the gate is still met, just by tests built new in this phase rather than pre-existing ones surviving unmodified. `cargo check` clean, 332 tests passing (0 failed, 0 skipped), clippy clean from a genuine cold build. | A clean window's CPU-side draw-issuing work is O(layer slots), independent of resident primitive count, measured directly (same `render_stats`-style counters R-N used, ported into `wgpui-core`) — **met**: counters equal across a 62× primitive-count difference across all four draw modes this adapter makes available, `instances_known_to_cpu == None` on every indirect path, and a steady-state sweep confirming the draw-plan caching fix holds (flat 1.3–2.1µs from 256 to 102,400 primitives; rises 0.4–7.6µs with layer count, as it should). **`WgpuSurface` check**: a viewport panel fully covered by a modal issues zero draws for its embedded 3D content, and `SurfaceRegistry`'s concurrency behavior is provably unaffected by the consumer-side unification — **met**: a covered viewport culls 1 of 2 entries to 1 composite draw with the producer's frame left unconsumed, an uncovered one restores both, the ported model's six tests are unmodified, a 13-step differential matches step-for-step, and a threaded run (200 composites / 172 frames / 6,191 backpressure skips) held under real concurrency — but see the correction at left: proven with tests built for this phase, not pre-existing ones. |
| **4.5** | `Buffering::Tiled` (§4.3): `LayerKey` extended to `(boundary, TileCoord)` addressing (already in place since Phase 1); tile-visibility compute pass driving indirect draw-arg generation for the in-range tile set (reusing Phase 4's mechanism directly); spatial mark-and-sweep eviction plus a total resident-tile budget with LRU eviction beyond it. **Executed** — merged to `2.0`. The tile-visibility pass writes byte-identical slot output to Phase 4's own layout (`[base, count, 0, 0]`), so an out-of-range tile is just a zero-count slot Phase 4's existing compaction already handles — no new draw path. §4.3's open "pick a multi-tile-content rule" question is resolved: the naive rule (anything spanning tiles goes to the overlay) **failed its own gate when actually measured** — 73% of a real scene ended up on the unbuffered overlay, because content up to one tile wide routinely straddles a tile boundary depending on where it happens to sit. Shipped instead: a primitive no larger than one tile anchors to whichever tile holds its top-left corner (overhang included); only genuinely oversized content reaches the overlay. Tile size measured 128–1024px rather than assumed: 256px wins, and the actual binding constraint at small sizes was the overlay share exceeding the grid (21.8% of the scene falling out at 128px), not the draw-call overhead §9's risk table predicted — both are real, the risk table's guess was just incomplete. Verification caught a real layer leak (`Compositor::sweep` returned only a boundary's own layer, so an evicted tiled boundary's tile slabs outlived the scene — confirmed by reverting the fix and watching the regression test fail) plus two smaller correctness bugs. `cargo check`/`cargo metadata --locked` clean, 360 tests passing (0 failed, 0 skipped, all 4 GPU tests confirmed actually running via `--nocapture`), clippy clean from a genuine cold build with zero suppressions. `docs/phase-4.5-results.md` has the full account, including the overlay's honest 3-layer-not-2-layer accounting below. | Panning a node-graph-style canvas across a tile boundary renders only the newly-revealed tile(s) — measured directly, not inferred — while panning within the resident grid costs one `TRANSFORM` update per visible tile and zero render/reconcile/layout work anywhere. **Met, with one disclosed qualification**: crossing a boundary revealed exactly 6 of 42 tiles and wrote 75 primitives against 432 resident (set-equality asserted, not just a count); panning within the grid produced 42 visible tiles, 43 `TRANSFORM` updates (one per tile plus the overlay), 0 primitives written, 0 upload bytes, across 8 consecutive frames, with every touched layer asserted to carry `TRANSFORM` *and nothing else*. The qualification: a tiled boundary has three kinds of layer, not two — the overlay itself can also re-render on a crossing (measured at ~10% of pan work, 70 primitives across 5 of 20 crossings in the same run). The gate's "only the newly-revealed tile(s)" is exactly true of the tile grid; the overlay sits outside that phrasing by construction and is reported separately rather than folded into the same number. |
| **5** | `wgpui-text`: shaped-run → patch conversion; `Img`/`StyledText` get the `diff_key` R-N Phase 7 left undone — the first two elements checked off against §6.2's standing invariant, not a one-off fix (closing SFD §3). Atlas-eviction subscription for GPU-resident glyph/sprite tiles (R-N §4.3's hazard, same fix, new home). **Executed, third attempt** (the first two died during their own reading pass — one user-paused for usage limits, one an API session-limit error — before either committed anything; nothing was lost either time because nothing existed yet, which is itself the argument for pushing early rather than only often). Merged to `2.0`. Real `cosmic-text` shaping (following the legacy `layout_line` recipe step for step), a device-free `etagere` atlas port (`AtlasTileId` fits exactly in the 4 bytes of tail padding Phase 1's `GlyphRun` left unused — not a coincidence forced after the fact, a prediction that paid off), and both `diff_key`s. `Img`'s absence in the legacy crate turned out to be an *ordering* problem, not a missing comparison — its `frame_index`/load state was only discoverable after `diff_key` was asked; 2.0's `Description` holds resolved state upfront, so the fix is architectural, not a cleverer comparison. `StyledText` splits color out of its comparison entirely, since `GlyphRun` carries color as a value — a recolor is `DISPLAY` over unchanged glyph positions, never a re-shape (measured: 2 emissions, 0 shaping). Atlas eviction scans resident primitives rather than maintaining a separate index, deliberately — an index has update sites that can be missed, and a missed one silently reproduces the exact bug the mechanism exists to prevent. Verification caught a real instance of exactly that class of bug in this phase's own atlas port (page-index reuse after eviction, caught by its own test, with the doc comment directly above the bug asserting the opposite of what the code did). 459 tests passing, clean check/clippy from a cold build, `src/` and this document both confirmed untouched by `git diff`. **Two things disclosed, not glossed over**: §6.2 is only **partially** discharged — neither element got `estimated_size`, only `diff_key` — and **nothing yet rasterizes a glyph's actual pixels into an allocated atlas tile**; shaping produces positions and atlas tile *requests*, the atlas allocates *space* for them, but the CPU-side step that renders a font outline into that space was out of this phase's scope and doesn't exist anywhere in `2.0` yet. Text is fully reconciled and fully positioned; it does not yet visibly draw. | Scroll-content-heavy scenes (avatars, multi-run text — SFD §3's stated motivation) hit the fast path with no per-refill shaping cost for unchanged rows, under ambient reconciliation (Phase 1), not because they're inside a `.boundary()` — **met, falsified both directions, not just satisfied on the happy path**: a differential test measures 80 lines shaped / 0 cache hits on a cold frame, then 0 lines shaped / 0 emissions / 0 upload bytes on two identical subsequent frames — the zero *cache hits* is what proves reconciliation skipped the work rather than a cache merely absorbing it. Removing `StyledText`'s `diff_key` turns the 0 cache hits into 80 (test fails); removing `Img`'s turns 0 emissions into 40 (test fails) — both checked by actually deleting the mechanism and watching the gate break, not by inspection. |
| **5.5** | *(Not in the original phase table — a gap Phase 5 itself disclosed rather than left implicit: shaping and atlas allocation existed, but nothing rasterized a glyph's font outline into the pixels its allocated tile was supposed to hold.)* Port the legacy `cosmic_text::SwashCache`-based rasterizer (`src/platform/cross/text_system.rs`'s `rasterize_glyph`), give the atlas real texel storage instead of just reserved rectangles, and upload rasterized bitmaps to the GPU. **Executed** — merged to `2.0`. The oracle for agreement is a **transcription** of the legacy rasterizer, not a live call to it — `RenderGlyphParams`/`TextSystem::rasterize_glyph` are `pub(crate)` in `gpui-ce`, unreachable without editing frozen `src/`, so a bug present in both the legacy code and its transcription would not be caught by this differential. Disclosed as a real methodological limit, not elided. Caught a genuine latent bug in Phase 5's own caching key: `GlyphRasterKey` folded scale factor into `font_size_bits`, correct for the bitmap's *size* but silently wrong for its *sub-pixel offset* (the legacy divides the offset by scale before binning it) — 16px@2× and 32px@1× collapsed onto one cache key that should have produced two different rasters. Fixed by adding the missing field. Separately, and deliberately: one legacy behavior that reads as a bug was **ported as-is, not fixed** — at 2× scale the four sub-pixel variants alias down to three distinct rasters, because reproducing legacy output exactly is the actual goal while both backends coexist; flagged explicitly in the report for revisit once that reason stops applying, rather than quietly "improved" out from under the parity goal. `cargo check` clean, 493 tests passing, clippy clean from a genuinely cold build (113 units, zero suppressions), `src/`/`docs/gpu-native-architecture.md`/root `Cargo.toml` all confirmed untouched by `git diff`. **Honest limit carried forward, not narrowed by this phase**: `2.0` now produces correct glyph pixels on the GPU and still does not put them on a screen — no sprite render pipeline, no sprite primitive kind wired to draw them. Phase 5's disclosure of this stands; Phase 5.5 fixed the half under it (pixels exist and are correct), not the half above it (something draws them). | Rasterized output matches the legacy renderer's, proven by an actual differential, not inspection — **met**: 2,592 requests compared byte-for-byte (72 glyph ids × 3 sizes × 3 scale factors × 4 sub-pixel variants), 0 disagreements, both the disagreement count and the "produced a bitmap at all" count asserted so the test can't pass vacuously. Falsified three separate ways to confirm the harness actually discriminates: flipping the bearing sign broke 2,556 of 2,592 comparisons; shifting the GPU upload origin by one texel failed 2 of 4 GPU tests at the exact affected texel (both reverted after confirming); dropping scale factor from the key broke 142 of 216 in a standing regression test. End-to-end on a real adapter (RTX 4060, Vulkan, 561.03, verified not assumed): 41 glyphs shaped → 30 rasterized + 5 atlas cache hits → 30 rectangles uploaded → every atlas page reads back identical to what was written, and every glyph claiming a tile finds real ink in it. |
| **5.6** | *(Not in the original phase table — closes the compositing half of the gap Phase 5.5 narrowed but left open: correct glyph pixels existed in GPU atlas memory with nothing sampling them.)* A third render pipeline, `MonoSpritePipeline`, following `QuadPipeline`'s existing storage-buffer vertex-pulling/indirect-draw pattern for `GlyphRun`/`Glyph` — text-only; images/`poly_sprites` and the other five legacy pipelines are explicitly deferred, not attempted. **Executed** — merged to `2.0`. **Two corrections to how this gap was described**, both found by the agent checking rather than trusting the brief: `mono_sprites.wgsl` was not "already ported, unused" as stated — it was a two-line placeholder like every shader in that directory except `quads`/`surfaces`, so the shader is this phase's own design work, not a port; and §9's "no sprite primitive kind" clause was simply wrong — `GlyphRun`/`Glyph` has been exactly that since Phase 1, so only the pipeline half of the gap was real. A genuine multi-page-atlas design problem (a bind group can't change mid-draw, so a slot whose glyphs span two atlas pages can't be one draw call) was solved by drawing the slot once per live page with the shader collapsing other pages' glyphs to degenerate strips — cost is honestly O(layer slots × live monochrome pages), reported via a counter rather than hidden. Verification found a real latent Phase 4 bug: `SlabBuffer::upload` never filtered `UploadRange` by primitive kind — invisible with only `Quad`'s single arena, silent corruption waiting for a second one, found by reading before wiring rather than by the corruption actually happening. `cargo check --all-targets` clean, 408 tests passing, clippy clean from a genuine cold recompile. `src/` and this document untouched. **The gap this phase actually closes is narrower than "text renders," and that's stated plainly, not implied away**: proof is byte-exact readback comparison against the atlas's own texel bytes (white-on-black reduces a rendered pixel to an identity with the source texel, so tests assert equality, not a threshold) — real, strong, and specifically *not* a screen, because **no window/present path exists anywhere in 2.0 yet**. That is a larger, more fundamental gap than anything about text specifically, surfaced here rather than assumed to already exist. | Visual correctness proven directly, not asserted — **met** for what a headless readback can prove: 3,097 texels compared on real shaped text, 3,920 across a forced 2-page split, all four draw modes byte-identical, a synthetic distinct-texel ramp specifically constructed to catch addressing errors smooth glyphs would hide, blanks and missing-atlas-tile cases covered. Disclosed limits, not silently narrowed: monochrome only, scale factor 1 only, positions rounded (since `wgpui-text` doesn't floor the pen the way the legacy renderer does), and — the one that matters most — nobody has looked at rendered output on an actual screen, because 2.0 has no window/present path to look at yet. |
| **6** | Window and present integration: winit event loop, `wgpu::Surface` creation/configuration bound to a real window, resize, and the actual `Description`→`Reconciler`→emission→`ScenePatch`→compute→indirect-draw pipeline driving a continuous per-frame loop — the first phase where anything in `2.0` is shown running rather than proven headlessly or by offscreen readback. **Executed** — merged to `2.0`. Interrupted once by a full process exit (not an API limit) with its report never written; the wrap-up wasn't a formality — the gate test **had never actually passed**: it asserted "exactly one `Surface::configure` per resize" and measured 15 for 8, because the test resizes the swapchain while the window stays fixed, and Vulkan's `OUT_OF_DATE` recovery on that mismatch was being counted as extra configures. Proved, not inferred, by reverting a manual commit and getting byte-identical numbers. Two further real fixes: a latent stale-clip bug where a shrink-then-grow-back resize kept the shrunk frame's occlusion decisions (reasoned, not observed — this phase's own scene can't exhibit it, disclosed as such); and, in a manually-authored commit made by the project owner testing the frame loop directly, **`present_mode: Immediate` was hardcoded without capability checking — `UnsupportedPresentMode` panics inside `WindowSurface::new` on any backend that doesn't support it** (WebGPU exposes only `Fifo`), fixed with a capability-checked preference list. A real window was confirmed visible in this environment (no offscreen fallback needed). 506 tests passing, clippy clean from a genuine cold build across all six `2.0` crates, zero suppressions. `docs/phase-6-results.md` has the full account, including the precise shape of the `Img`/image gap this phase confirmed rather than guessed at: **four missing layers, not one** — no image kind in `PrimitiveKind`, no polychrome tile producer, no pipeline, and no image loading or decoding anywhere in any `2.0` crate at all. | All four milestones (window presents; resize survives; the real pipeline drives the loop; on-screen output is pixel-exact) — **met**, with the gate test itself corrected to assert against `configures − retries` rather than raw configure count. 20/20 frames verified for the clear-color proof (3,200,000 pixels byte-exact); 66/66 frames across 9 real WM-driven reconfigurations for resize (0 retried, 0 lost, 0 skipped, 160×100 to 1280×800); the full pipeline confirmed driving real `Quad`/`GlyphRun` content through real cosmic-text shaping, not a bypassed scene; 4,373 glyph texels and 57,600 quad pixels byte-exact against expected values, read off the actual presented swapchain image. |
...
| **6.2** | **Confirmed by Phase 6's verification, not assumed: four missing layers, not one.** No `Img`/image primitive kind exists in `wgpui-core::patch::primitive::PrimitiveKind` at all (unlike text, which had `GlyphRun` since Phase 1); no polychrome atlas-tile producer; no `PolySpritePipeline`; and no image loading or decoding anywhere in any `2.0` crate — no `image`/PNG/JPEG dependency exists yet in `wgpui-text`, `wgpui-widgets`, or `wgpui-wgpu` (legacy's `image` crate dependency is in the frozen `src/` tree, not reachable from `2.0`). All four need building, in that order — primitive kind first, the same way `GlyphRun` had to exist before anything could produce or draw one. SVG support piggybacks on this once the polychrome path exists: `resvg`/`usvg` rasterize to a bitmap on CPU exactly as legacy does (§6's "stays on CPU" reasoning already covers this, it was never in question), uploaded through the same polychrome tile path as any other image. Directly blocks example parity (§7): `image_loading`, `gif_viewer`, `svg`, and any example using `img()`/`svg()` cannot run without all four layers existing. | A real image file loads, decodes, uploads, and renders byte-exact against the legacy renderer's output for the same source — same differential-proof discipline as Phase 5.6, one primitive kind over, now genuinely built from zero rather than extending something partial. |
| **6.3** | `ShadowPipeline` + `UnderlinePipeline` — both flagged in Phase 5.6's own report as `QuadPipeline`-shaped, i.e. genuinely cheap once the pattern is proven three times over (`Quad`, `GlyphRun`, and now `Img`'s sprite). Directly blocks example parity: `shadow` (bench), `window_shadow`, and any text with underline/strikethrough styling. | Both pipelines byte-exact against legacy output, same discipline, reusing the now-three-times-proven pattern rather than re-deriving it. |
...
| **Resolved in Phase 4.5, kept as a record of what was actually true vs. predicted**: `Buffering::Tiled` (§4.3) picks a tile size that's wrong for a given workload. Predicted failure mode at the small end was per-tile/draw-call overhead; measured failure mode was overlay overflow (21.8% of the scene falling out of the grid at 128px, because ordinary content exceeds a small tile). 256px won (11.3× vs. refill, 2.8% overlay) and is the shipped default. An erratic pan pattern keeping more tiles "recently visited" than a per-tile timer alone bounds was mitigated as planned | Tile size measured 128–1024px against a real workload rather than asserted from compositor folklore (Phase 4.5). Total resident-tile budget with LRU eviction shipped as a first-class part of the mechanism (`over_budget()` reports rather than silently absorbs an unmeetable budget), proven by comparison against a timer-only arm, not by a number happening to stay small in testing. |
| **Discovered during Phase 1, not assumed**: `PrimitiveStore::reflow` (`wgpui-core::scene`) is O(n) per insert/remove, so building a scene by repeated appends is O(n²) — measured directly as 2.6× the final layer size uploaded during a 10,000-quad bulk build, from size-class relocations. The O(1) value-update path (§5.0's actual steady-state guarantee) doesn't go through it, so this doesn't violate Phase 1's gate, but Phase 3's 100K-primitive scale (validated by Spike A for *ordering/occlusion*, not for *population*) could hit this hard if a scene is ever built by naive one-at-a-time appends rather than a bulk-insert path | Not yet mitigated — flagged for whichever phase first needs to construct large scenes at speed (Phase 3 or the widget work that precedes it). A bulk-insert/reserve API on `PrimitiveStore` that pre-sizes before reflowing is the likely fix; add it when a real workload needs it, per this document's own measure-before-building discipline, not preemptively. |
| **Discovered and measured in Phase 3**: GPU compute occlusion *loses* to the CPU path on scenes where only a small fraction of primitives is actually visible (measured: 1.30× slower on a 99,054-primitive scene where 3% is visible) — the CPU early-exits on the 97% invisible majority per-primitive, while the GPU pays a fixed upload+dispatch cost regardless of how much of that work turns out to be discarded | Not yet mitigated. Two directions, neither built: a cheap CPU-side pre-filter (bounding-box-only, not the full conservative test) that skips upload entirely for primitives trivially outside the viewport before the compute pass ever sees them; or accept the loss for this specific shape and let per-layer heuristics (already planned for `rasterize_above`-style decisions, R-N §3.3) route very-sparse layers to the CPU path instead of GPU. Address when a real low-visibility workload (a zoomed-out node graph is the obvious candidate, tying directly to §4.3's tiling motivation) actually needs it — not preemptively. |
| **Resolved by Phases 5.5 and 5.6 — closed, not merely narrowed, and one part of the original description was wrong.** Text is shaped, positioned, reconciled, rasterized, and now actually drawn: `MonoSpritePipeline` samples the atlas and produces byte-exact-verified pixels. The gap's original framing ("no sprite primitive kind") was itself inaccurate — `GlyphRun`/`Glyph` existed as exactly that since Phase 1; only the render-pipeline half was ever really missing. Recorded here as a correction, not silently edited away, the same discipline this document asks of every phase's own claims | No action needed — this row exists as a record that the description was fixed once checked, not to track open work. |
| **Resolved by Phase 6.** `2.0` now has a real window/present path — this row is kept as a record that the gap was named and closed, the same discipline as every other resolved row in this table | No action needed. |
...
Phases 0–6 are merged to `2.0`. Write-ups: `docs/phase-0-results.md`
through `docs/phase-6-results.md`. `2.0` now has a real window/present path
— winit, a real `wgpu::Surface`, real resize, and the actual
reconciler→emission→compute→indirect-draw pipeline driving a continuous
loop, proven pixel-exact off the real presented swapchain image. This is
the first point across the whole rearchitecture where something has
genuinely been seen running, and the phase's own wrap-up is why: the gate
test had never actually passed as pushed, and a hardcoded, uncapability-
checked present mode would have panicked on any backend other than the one
development happened on (WebGPU exposes only `Fifo`) — both real, both
caught before merge, not after. The target now is exact example parity
(§7): every example in the repository, crate name as the only permitted
change. The concrete next steps, in order:

1. **Phase 6.2 (images) is the next hard blocker for example parity.**
   Confirmed, not assumed: four missing layers, not one — no image
   primitive kind, no polychrome tile producer, no pipeline, no image
   loading/decoding anywhere in `2.0` at all. Build in that order, the same
   way `GlyphRun` had to exist before Phase 5 could shape into it and
   Phase 5.6 could draw it.
2. Then 6.3 (shadows/underlines — flagged cheap, `QuadPipeline`-shaped) and
   6.4 (paths/backdrop-blur), each closing specific named examples per
   their rows in §8.
3. Decide Phase 6.1's fate for real: run the fused-dispatch follow-up spike
   sketched in its rescoped row (§8), or drop the phase outright and remove
   it from the table rather than leaving it permanently open. Lower
   priority than 6.2-6.4 — it's a performance question, not a parity
   blocker.
4. Re-run Phases 0, 3, 4, 4.5, and 6's benchmarks/window behavior on at
   least one non-NVIDIA/non-Windows configuration (integrated GPU,
   macOS/Metal, Linux/Vulkan) before treating any of these results as
   settled — every number and every window-present claim so far is one
   machine, one driver, one backend, one OS.
5. Then §8's remaining phases in order: devtools extraction (7), the legacy
   alias crate + full parity run (8, §3.7/§7's sharpened bar).
6. Carried-forward, not urgent: `PrimitiveStore::reflow`'s O(n²) bulk-build
   cost, GPU occlusion's 1.30× loss on low-visibility scenes, neither `Img`
   nor `StyledText` having `estimated_size`, the transcription-oracle limit
   on Phase 5.5's differential, the deliberately-preserved 2×-scale
   sub-pixel aliasing quirk, Phase 5.6's disclosed narrowness (monochrome
   only, scale factor 1 only, unrounded pen position), and Phase 6's
   reasoned-not-observed stale-clip fix (§9, all measured or named, none
   yet mitigated because none yet need to be) — address each when a real
   need arrives. `gpui-ce`'s own legacy test binary still hasn't been
   confirmed to finish (unmodified by any 2.0 branch per every phase's
   `git diff`, but "very likely pre-existing" and "confirmed" remain
   different things) — let it run to completion once, unhurried.

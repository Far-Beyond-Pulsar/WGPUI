...
Phases 0–3 are merged to `2.0`. Write-ups: `docs/phase-0-results.md`
through `docs/phase-3-results.md`. Phase 1's named seam (nothing produced a
`Description`/`ScenePatch` from a real element) was closed inside Phase 2
itself. Phase 3 is the first phase on a live `wgpu::Device`, and the first
whose numbers are genuinely scene-shape-dependent rather than a clean
multiplier — see its row in §8 before quoting the 84× out of context; 1.41×
is the number this plan actually rests on. The concrete next steps, in
order:

1. Re-run Phase 0's and Phase 3's benchmarks on at least one
   non-NVIDIA/non-Windows configuration (integrated GPU, macOS/Metal,
   Linux/Vulkan) before treating any of these results as settled — every
   number so far is one machine, one driver, one backend.
...
3. Start Phase 4 (indirect draw-arg generation + `multi_draw_indirect`,
   §5.3, plus the `WgpuSurface`/`SurfaceRegistry` compositing unification
   named in §5.5's Gap 2) — the natural continuation of Phase 3's compute
   passes: they currently compute ordering/occlusion decisions that nothing
   yet turns into indirect draw arguments.

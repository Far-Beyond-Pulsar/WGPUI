...
| **Resolved by Phase 5.5's rasterization half, narrowed rather than closed**: text is now shaped, positioned, reconciled, rasterized, and correctly present in GPU atlas texture memory (proven by an actual differential and a real page readback, not inspection) — but nothing yet draws it: there is no sprite render pipeline and no sprite primitive kind consuming those atlas tiles. The gap named in Phase 5's own disclosure ("does not yet visibly draw") is exactly as true after Phase 5.5 as before it; only the boundary of what's missing moved from "no pixels exist" to "correct pixels exist and nothing samples them" | Same disposition as before, narrower now: needs an explicit decision, not an assumption a later phase absorbs it incidentally. Fold sprite rendering into whichever phase first needs real on-screen output, or give it its own small phase — flagged in §11, not silently deferred. |
...
Phases 0–5.5 are merged to `2.0`. Write-ups: `docs/phase-0-results.md`
through `docs/phase-5.5-results.md`. Phase 3's numbers are genuinely
scene-shape-dependent — see its row in §8 before quoting the 84× out of
context; 1.41× is the number this plan actually rests on. Phase 4.5
disproved its own first guess at the multi-tile-content rule by actually
measuring it. Phase 5 gave `Img`/`StyledText` real `diff_key`s, falsified
in both directions. Phase 5.5 (not in the original table — a gap Phase 5
disclosed rather than hid) made glyph pixels real and correct in GPU atlas
memory, proven by an actual differential against a transcribed legacy
oracle. **The rasterization half of "text doesn't render" is closed; the
compositing half — no sprite pipeline, nothing samples the atlas tiles at
all — is not, and is now the single most load-bearing open item**: it's
the first point across seven phases where "every gate passed" and
"something appears on a screen" have genuinely diverged. The concrete next
steps, in order:

1. **Give sprite rendering — the pipeline that actually samples the atlas
   and draws a glyph or image sprite — an explicit home now.** Every phase
   through 5.5 was correctly scoped to prove its own mechanism without
   needing visible output, but that reason runs out here: there is now
   correct pixel data sitting in GPU memory with nothing pointed at it. Fold
   it into the next phase with its scope stated up front, don't let it drift
   into an implicit assumption the way glyph rasterization briefly did.
...
4. Carried-forward, not urgent: `PrimitiveStore::reflow`'s O(n²) bulk-build
   ...
   on Phase 5.5's differential, and the deliberately-preserved 2×-scale
   sub-pixel aliasing quirk (§9, all measured or named, none yet mitigated
   because none yet need to be) — address each when a real need arrives.
   `gpui-ce`'s own legacy test binary still hasn't been confirmed to finish
   (unmodified by any 2.0 branch per every phase's `git diff`, but "very
   likely pre-existing" and "confirmed" remain different things) — let it
   run to completion once, unhurried.

...
Phases 0–4.5 are merged to `2.0`. Write-ups: `docs/phase-0-results.md`
through `docs/phase-4.5-results.md`. Phase 1's named seam (nothing produced
a `Description`/`ScenePatch` from a real element) was closed inside Phase
2. Phase 3's numbers are genuinely scene-shape-dependent — see its row in
§8 before quoting the 84× out of context; 1.41× is the number this plan
actually rests on. Phase 4 made indirect draw and the `WgpuSurface`/
`SurfaceRegistry` unification real. Phase 4.5 completed the compute
pipeline this document scoped for tiled 2D content — and along the way
disproved its own first guess at the multi-tile-content rule by actually
measuring it (§4.3), which is the discipline working as intended, not a
setback. The concrete next steps, in order:

1. Re-run Phases 0, 3, 4, and 4.5's benchmarks on at least one non-NVIDIA/
...
2. Decide Phase 6.1's fate for real: run the fused-dispatch follow-up spike
...
3. Start Phase 5 (`wgpui-text`: shaped-run → patch conversion; `Img`/
   `StyledText` finally get the `diff_key` §6.2's standing invariant has
   required since Phase 1, closing SFD §3's own named gap).
4. Carried-forward, not urgent: `PrimitiveStore::reflow`'s O(n²) bulk-build
   cost and GPU occlusion's 1.30× loss on low-visibility scenes (§9, both
   measured, neither yet mitigated) — address when a phase's real workload

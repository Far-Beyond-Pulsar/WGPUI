...
Phases 0–6.2 are merged to `2.0`. Write-ups: `docs/phase-0-results.md`
through `docs/phase-6.2-results.md`. Images now decode, upload, and render
byte-exact against legacy output — the third primitive kind, following
`Quad` and `GlyphRun`'s established pattern exactly. Two real gaps surfaced
along the way, not discovered later: image scaling is nearest-neighbor
where legacy interpolates (§9), and **`.with_animation()` doesn't exist
anywhere in `2.0`** — confirmed, not assumed, and significant enough to get
its own phase (6.5) rather than a footnote, since it's frontend-frozen
public API blocking multiple examples directly, not an internal detail.
The concrete next steps, in order:

1. 6.3 (shadows/underlines — flagged cheap, `QuadPipeline`-shaped) and 6.4
   (paths/backdrop-blur), each closing specific named examples per their
   rows in §8.
2. **6.5 (the animation driver)** — real per-frame interpolation over style
   deltas, driven by `request_animation_frame` (real since Phase 6), under
   the frozen `.with_animation()`/`AnimationExt` API. Closes the `animation`
   example, likely `smooth_scrolling`/`karaoke_*`, and GIF frame
   advancement (Phase 6.2's decode-without-animate gap) in one pass rather
...
3. Decide Phase 6.1's fate for real: run the fused-dispatch follow-up spike
   ...
   priority than 6.2-6.5 — it's a performance question, not a parity
   blocker.
...
6. Carried-forward, not urgent: `PrimitiveStore::reflow`'s O(n²) bulk-build
   ...
   nor `StyledText` having `estimated_size` (`Img::layout_size()` returning
   decoded natural size is real progress but a distinct mechanism, not this
   one), the transcription-oracle limit on Phase 5.5's differential, the
   deliberately-preserved 2×-scale sub-pixel aliasing quirk, Phase 5.6's
   disclosed narrowness (monochrome only, scale factor 1 only, unrounded
   pen position), Phase 6's reasoned-not-observed stale-clip fix, and image
   scaling's nearest-neighbor gap (§9, all measured or named, none yet

...
| **Load-bearing, disclosed by Phase 5 rather than found later by someone wondering why nothing renders**: text is fully shaped, positioned, reconciled, and its atlas tiles are allocated — but nothing anywhere in `2.0` yet rasterizes a glyph's font outline into the pixels that allocated tile is supposed to hold. `wgpui-text` produces positions and tile *requests*; `wgpui-wgpu`'s atlas allocates *space*; the CPU-side rasterization step in between (what the legacy crate gets from its existing font/rasterizer stack) was out of Phase 5's stated scope and was never separately scoped into any phase | Needs an explicit decision, not a default assumption that a later phase absorbs it incidentally: either fold glyph rasterization into whichever phase first needs real on-screen pixel output (plausibly the cutover-adjacent work, Phase 7/8), or give it its own small phase now rather than discovering the gap when something is expected to visibly draw and doesn't. Flagged in §11 rather than silently deferred. |
...
Phases 0–5 are merged to `2.0`. Write-ups: `docs/phase-0-results.md`
through `docs/phase-5-results.md`. Phase 1's named seam was closed inside
Phase 2. Phase 3's numbers are genuinely scene-shape-dependent — see its
row in §8 before quoting the 84× out of context; 1.41× is the number this
plan actually rests on. Phase 4.5 disproved its own first guess at the
multi-tile-content rule by actually measuring it, which is the discipline
working as intended. Phase 5 gave `Img`/`StyledText` real `diff_key`s,
falsified in both directions, not just satisfied on the happy path — but
left two things genuinely open, not silently: §6.2 is `diff_key`-complete
only, not `estimated_size`-complete, for either element; and **nothing yet
rasterizes a glyph's pixels into its allocated atlas tile** — text is fully
positioned and reconciled but does not yet visibly draw (§9's newest risk
row). The concrete next steps, in order:

1. **Decide the glyph-rasterization gap's home now, not incidentally.**
   This is the first point in the whole plan where "everything's tested and
   green" and "something actually renders" have diverged — every phase
   through 5 proved its gate headlessly or against resident GPU data, and
   that was correct for what each was building, but it means nobody has
   pointed at a screen yet. Give it an explicit phase or fold it into
   whichever comes next with the scope stated up front.
...
4. Carried-forward, not urgent: `PrimitiveStore::reflow`'s O(n²) bulk-build
   cost, GPU occlusion's 1.30× loss on low-visibility scenes, and neither
   `Img` nor `StyledText` having `estimated_size` (§9, all measured or
   named, none yet mitigated) — address each when a phase's real workload
   needs it. `gpui-ce`'s own legacy test binary still hasn't been confirmed
   to finish (unmodified by any 2.0 branch per every phase's `git diff`, but
   "very likely pre-existing" and "confirmed" remain different things) —
   let it run to completion once, unhurried.

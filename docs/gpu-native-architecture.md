...
Phases 0–4 are merged to `2.0`. Write-ups: `docs/phase-0-results.md`
through `docs/phase-4-results.md`. Phase 1's named seam (nothing produced a
`Description`/`ScenePatch` from a real element) was closed inside Phase 2.
Phase 3's numbers are genuinely scene-shape-dependent rather than a clean
multiplier — see its row in §8 before quoting the 84× out of context; 1.41×
is the number this plan actually rests on. Phase 4 completes the compute
pipeline `.boundary()` and Phase 3 needed: indirect draw is real, and
`WgpuSurface`/`SurfaceRegistry` composite through the same path a boundary's
retained texture does. The concrete next steps, in order:

1. Re-run Phases 0, 3, and 4's benchmarks on at least one non-NVIDIA/
...
3. Start Phase 4.5 (tile-based buffering for freeform 2D content, §4.3) —
   the phase this document's own §4.3 says depends on addressable layers
   and GPU-computed draw-arg generation being real, both now true.

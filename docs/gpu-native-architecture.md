...
Phases 0–2 are merged to `2.0` (PR #149; #150; #152/#153 + a wrap-up
commit). Write-ups: `docs/phase-0-results.md`, `docs/phase-1-results.md`,
`docs/phase-2-results.md`. Phase 1's named seam (nothing produced a
`Description`/`ScenePatch` from a real element) was closed inside Phase 2
itself rather than requiring a separate phase-table addition — the minimal
emission path both gates needed doubled as that element vocabulary's first
real exercise. The concrete next steps, in order:

1. Re-run both Phase 0 spikes on at least one non-NVIDIA/non-Windows
   configuration (integrated GPU, macOS/Metal, Linux/Vulkan) before treating
   either result as settled — still one machine, one driver, one backend.
...
3. Start Phase 3 (GPU compute ordering + occlusion, §5.1/§5.2): the first
   phase to touch a live `wgpu::Device`, in `wgpui-wgpu`. Its job is
   explicitly named in the scaffold's own doc comments — turn Phase 0's
   `spike_a_ordering_occlusion.rs` prototype into the real pass consuming
   `wgpui-core`'s actual `Scene`/`Layer`/`PrimitiveStore` (not a synthetic
   100K-quad benchmark), and implement `occlusion/coverage.rs`'s conservative
   opaque-region test (R-N §8.3), still a Phase 0 stub.
4. Carried-forward, not urgent: the `PrimitiveStore::reflow` O(n²)-bulk-build
   finding (§9) doesn't block anything yet — address it when a phase
   actually needs to construct a large scene at speed, which Phase 3 might
   be the first to. `gpui-ce`'s own legacy test binary still hasn't been

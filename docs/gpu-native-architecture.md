...
| **6.2** | Image loading + `PolySpritePipeline`: real decode (the `image` crate, already a dependency) → texture upload → a fourth render pipeline following `QuadPipeline`/`MonoSpritePipeline`'s established pattern, plus SVG support (`resvg`/`usvg` rasterize to a bitmap on CPU, exactly as legacy does — §6's own "stays on CPU" reasoning already covers this, it was never in question) uploaded through the same path. This is not just a missing pipeline the way glyph rendering was — `Img` has no image-loading/decode/upload path anywhere in `2.0` yet, confirm the exact shape of that gap before scoping the fix, don't assume it's pipeline-only. Directly blocks example parity (§7): `image_loading`, `gif_viewer`, `svg`, and any example using `img()`/`svg()` cannot run without this. | A real image file loads, decodes, uploads, and renders byte-exact against the legacy renderer's output for the same source — same differential-proof discipline as Phase 5.6, one primitive kind over. |
...
| **Surfaced by Phase 5.6, and larger than anything about text specifically**: `2.0` has no window/present path anywhere. Every phase through 5.6 proved its work headlessly or via GPU readback comparison — correctly, for what each was building — but it means nothing in this entire rearchitecture has ever been looked at on an actual screen. `wgpui-wgpu`'s own `window/` subtree (`mod.rs`, `dispatcher.rs`, `keyboard.rs`, `resize_detector.rs`, `app_menu.rs`, §3.5's file map) is still exactly the Phase 0 stub it started as | Needs its own explicit scope, not an assumption that final cutover (Phase 8) absorbs it as a formality. Real work: winit event loop integration, surface/swapchain configuration, resize handling, an actual entry point a developer can run. Comparable in size to Phase 1, not a footnote — flagged as the top item in §11 rather than left implicit until Phase 8 discovers it's still missing. |
...
Phases 0–5.6 are merged to `2.0`. Write-ups: `docs/phase-0-results.md`
through `docs/phase-5.6-results.md`. Phase 3's numbers are genuinely
scene-shape-dependent — see its row in §8 before quoting the 84× out of
context; 1.41× is the number this plan actually rests on. Phase 5.5 made
glyph pixels real and correct in GPU atlas memory; Phase 5.6 made a real
pipeline sample them and draw them, proven byte-exact against the atlas's
own bytes via readback. **Both were correctly, honestly scoped to prove
their work without a screen to look at — and that reason has now run out.
`2.0` has no window/present path anywhere**; nothing in seven-plus phases
of real, verified work has ever been looked at running. This is the
largest genuinely open item, not a footnote (§9). The concrete next steps,
in order:

1. **Scope window/present integration explicitly, sized like a real
   phase** (comparable to Phase 1, not a wrap-up task): winit event loop,
   surface/swapchain configuration, resize handling, an actual runnable
   entry point. `wgpui-wgpu`'s own `window/` subtree has sat as its Phase 0
   stub through every phase since. This is the first point where letting it
   ride further risks it being discovered as missing at cutover instead of
   planned for now.
2. Re-run Phases 0, 3, 4, and 4.5's benchmarks on at least one non-NVIDIA/
   non-Windows configuration (integrated GPU, macOS/Metal, Linux/Vulkan)
   before treating any of these results as settled — every number so far is
...
3. Decide Phase 6.1's fate for real: run the fused-dispatch follow-up spike
   ...
   it from the table rather than leaving it permanently open.
4. Carried-forward, not urgent: `PrimitiveStore::reflow`'s O(n²) bulk-build
   ...
   sub-pixel aliasing quirk, and Phase 5.6's own disclosed narrowness
   (monochrome only, scale factor 1 only, unrounded pen position) plus its
   deferred pipelines (`poly_sprites`/images — needs a new primitive kind,
   larger; `shadows`/`underlines` — flagged as `QuadPipeline`-shaped, cheap;
   `paths`/`backdrop_blur` — not yet scoped) (§9, all measured or named,
   none yet mitigated because none yet need to be) — address each when a
   real need arrives. `gpui-ce`'s own legacy test binary still hasn't been

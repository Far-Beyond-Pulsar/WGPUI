...
| **7** | `ElementInstance` + `reconcile` with the conservative default. Migrate `Div`, `Text`, `Img`, `Svg`. | Inside a dirty layer, unchanged subtrees skip layout/paint. Text with pointer-equal `SharedString` skips shaping. |
| **8** | Persistent Taffy: instances retain `LayoutId`; measure closures capture instance ids (§2.5). | `TaffyLayoutEngine::clear()` no longer called unconditionally; layout time scales with dirty nodes. |

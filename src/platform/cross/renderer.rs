                ...
                match batch {
                    ...
                    PrimitiveBatch::Surfaces(surfaces) => {
                        ...
                        for surface in surfaces {
                            let view = match &surface.content {
                                crate::SurfaceContent::Wgpu(surface_id) => {
                                    ...
                                    let view = self.context.surface_registry.front_view(*surface_id);
                                ...
                                crate::SurfaceContent::Texture { texture, size } => {
                                    log::info!("[COMPOSITOR PAINT] Processing texture surface {}x{}", size.width.0, size.height.0);

                                    // Downcast the type-erased handle to a concrete wgpu::Texture.
                                    // Callers are expected to pass an Arc<wgpu::Texture>; the Arc
                                    // coerces to Arc<dyn Any + Send + Sync>, so dereffing gives
                                    // dyn Any + Send + Sync whose inner type is wgpu::Texture.
                                    if let Some(tex) = texture.downcast_ref::<wgpu::Texture>() {
                                        Some(tex.create_view(&wgpu::TextureViewDescriptor::default()))
                                    } else {
                                        log::warn!("[COMPOSITOR PAINT] Texture surface has unrecognised concrete type, skipping");
                                        None
                                    }
                                }
                            };
                            ...
                            if let Some(view) = view {
                                log::info!("[COMPOSITOR PAINT] Got view for surface");

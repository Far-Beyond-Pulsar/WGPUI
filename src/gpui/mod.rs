...
/// The extensible render-primitive plugin API. Primitive types live in their own
/// crates depending only on `gpui-render-primitive`, register a [`RenderPrimitive`]
/// via [`Window::register_render_primitive`], and are painted with
/// [`Window::paint_custom_primitive`].
pub use gpui_render_primitive::{
    BatchDraw,
    PipelineContext,
    PrimBounds,
    PrimitiveTypeId,

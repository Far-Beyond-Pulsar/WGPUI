...
use crate::retained::MeasurementCache;
use crate::{
    AbsoluteLength,
    App,
    Bounds,
...
struct NodeContext {
    ...
    // Each taffy node owns its own measurement cache, so the cache key carries no
    // node identity; known dimensions and available space (tracked inside the
    // cache) fully identify a measure query for a given node.
        ...
        self.taffy
            .compute_layout_with_measure(
                ...
                |known_dimensions, available_space, _id, node_context, _style| {
                    ...
                    let a: Size<Pixels> = if let Some(cached) =
                        node_context
                            .measurements
                            .get((), known_dimensions, available_space)
                    {
                        ...
                        node_context.measurements.insert(
                            (),
                            known_dimensions,
                            available_space,
                            measured,

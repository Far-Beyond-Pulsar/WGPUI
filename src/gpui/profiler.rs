...
crate::actions!(
    gpui,
    [
        /// Toggle the UI refresh-heat overlay: a 2D analogue of Unreal's
        /// shader-complexity view that tints each view by how long it took to
        /// refresh, with a heat legend along the bottom. Bind this in a keymap and
        /// call [`Window::toggle_refresh_overlay`](crate::Window::toggle_refresh_overlay)
        /// from the listener.
...
pub struct FrameMetrics {
    ...
    /// Diagnostic only: number of id-bearing elements mirrored into the retained
    /// `FiberTree` this frame. The fiber tree is a read-only side table; it does
    /// not (yet) drive rendering, so this metric must not be used to gate redraw.
    pub retained_fiber_count: u32,
    /// Diagnostic only: of `retained_fiber_count`, how many fibers carried
    /// non-clean dirty flags after reconciliation against the previous frame.
    pub retained_dirty_fiber_count: u32,
    /// Hottest view refresh-cost level observed this frame (the refresh-heat
    /// overlay's whole-window summary).
    pub refresh_heat: crate::RefreshHeat,
    /// Number of custom (plugin) render-primitive instances queued this frame.
    pub custom_primitive_count: u32,
}

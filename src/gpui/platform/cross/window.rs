...
pub(crate) struct CrossWindowInner {
    ...
    /// Custom render-primitive plugins registered before the renderer exists.
    /// Flushed into the renderer the moment it is created (see `initialize`).
    pub(crate) pending_primitives: RefCell<Vec<Box<dyn crate::RenderPrimitive>>>,
}
...
impl CrossWindow {
    ...
    pub(crate) fn new(
        ...
        Self(Arc::new(CrossWindowInner {
            ...
            pending_primitives: RefCell::new(Vec::new()),
        }))
    }
    ...
    pub(crate) fn initialize(&self, winit_window: winit::window::Window) {
        ...
        if initial_size.width > 0 && initial_size.height > 0 {
            ...
            // Flush any render-primitive plugins registered before the renderer
            // existed, so their first frame is drawn.
            for primitive in self.0.pending_primitives.borrow_mut().drain(..) {
...
impl PlatformWindow for CrossWindow {
    ...
    fn register_render_primitive(&self, primitive: Box<dyn crate::RenderPrimitive>) {
        // If the renderer already exists, register directly; otherwise buffer the
        // registration and flush it when the renderer is created (see `initialize`).
        if let Some(renderer) = self.0.renderer.get() {

...
pub(crate) struct Frame {
    ...
    retained_node_ids_by_view: FxHashMap<EntityId, Vec<GlobalElementId>>,
    pub(crate) retained_fiber_tree: crate::retained::FiberTree,
...
impl Frame {
    pub(crate) fn new(dispatch_tree: DispatchTree) -> Self {
        Frame {
            ...
            retained_node_ids_by_view: FxHashMap::default(),
            retained_fiber_tree: crate::retained::FiberTree::default(),
    ...
    pub(crate) fn clear(&mut self) {
        ...
        self.retained_node_ids_by_view.clear();
        self.retained_fiber_tree = crate::retained::FiberTree::default();
    ...
    pub(crate) fn finish(&mut self, prev_frame: &mut Self) {
        ...
        self.scene.finish_with_previous_scene(&prev_frame.scene);
    }
}
...
pub struct Window {
    ...
    refresh_profiler: crate::retained::RefreshProfiler,
    refresh_overlay_enabled: bool,
...
impl Window {
    pub(crate) fn new(
        ...
        Ok(Window {
            ...
            frame_number: 0,
            frame_metrics: None,
            refresh_profiler: crate::retained::RefreshProfiler::default(),
            refresh_overlay_enabled: false,
            root: None,
...
impl Window {
    ...
    /// Whether the UI refresh-heat overlay is currently shown.
    pub fn refresh_overlay_enabled(&self) -> bool {
        self.refresh_overlay_enabled
    }

    /// Enable or disable the UI refresh-heat overlay: a 2D analogue of Unreal's
    /// shader-complexity view that tints each view by how long its most recent
    /// refresh took (green = cheap/reused, red = expensive), with a legend along
    /// the bottom. While enabled the window repaints every frame so the heat
    /// stays live.
    pub fn set_refresh_overlay(&mut self, enabled: bool) {
        if self.refresh_overlay_enabled != enabled {
            self.refresh_overlay_enabled = enabled;
            self.refresh();
        }
    }

    /// Toggle the UI refresh-heat overlay.
    pub fn toggle_refresh_overlay(&mut self) {
        self.set_refresh_overlay(!self.refresh_overlay_enabled);
    }

    /// Record how long a view's prepaint (layout + prepaint, or a cheap cache
    /// reuse) took this frame, for the refresh-heat overlay.
    pub(crate) fn record_view_refresh(
        &mut self,
        view_id: EntityId,
        bounds: Bounds<Pixels>,
        elapsed: std::time::Duration,
    ) {
        self.refresh_profiler.record(view_id, bounds, elapsed);
    }

    /// Inject the refresh-heat overlay primitives — one tinted quad per view
    /// drawn this frame plus a heat legend — on top of the current frame's
    /// scene. Called during [`Window::draw`] while the overlay is enabled.
    fn paint_refresh_overlay(&mut self) {
        ...
        let scaled_viewport = content_mask.bounds.scale(scale_factor);

        let mut overlay: Vec<(Bounds<Pixels>, Hsla)> = self
            .refresh_profiler
            .regions()
            .into_iter()
            .map(|(bounds, heat)| (bounds, heat.overlay_color()))
            .collect();

        // Legend: opaque swatches Cold -> Critical along the bottom-left edge.
        let swatch = size(px(36.), px(14.));
        let gap = px(6.);
        let mut swatch_origin = point(px(8.), viewport.height - swatch.height - px(8.));
        for heat in crate::retained::RefreshHeat::gradient() {
            overlay.push((
                Bounds {
                    origin: swatch_origin,
                    size: swatch,
                },
                heat.legend_color(),
            ));
            swatch_origin.x += swatch.width + gap;
        }

        // A dedicated top layer keeps the overlay above all painted content.
        self.next_frame.scene.push_layer(scaled_viewport);
        for (bounds, color) in overlay {
    ...
    /// Register a custom render-primitive plugin with this window's renderer.
    ///
    /// The primitive's queued instances (painted via
    /// [`Window::paint_custom_primitive`]) are drawn batched — one GPU call per
    /// type — after the built-in primitives each frame. Registration before the
    /// renderer exists is buffered and applied once the renderer is created; on
    /// platforms without a GPU renderer (e.g. the test platform) it is a no-op.
    pub fn register_render_primitive(&self, primitive: Box<dyn crate::RenderPrimitive>) {
    ...
    pub fn draw<'app>(&mut self, cx: &'app mut App) -> ArenaClearNeeded<'app> {
        ...
        // Refresh-heat overlay: start a fresh per-frame heat record, and while the
        // overlay is shown force a full repaint so every view's refresh cost is
        // measured and the overlay covers the whole window.
        self.refresh_profiler.begin_frame();
        if self.refresh_overlay_enabled {
            self.refreshing = true;
        }
        ...
        if self.refresh_overlay_enabled {
            self.paint_refresh_overlay();
            // Keep the heat live by scheduling another frame.
            self.on_next_frame(|window, _| window.refresh());
        }
        ...
        let retained_fiber_tree = &self.rendered_frame.retained_fiber_tree;
        let retained_fiber_count = retained_fiber_tree.fiber_count() as u32;
        let retained_dirty_fiber_count = retained_fiber_tree.dirty_fiber_count() as u32;
        let refresh_heat = self.refresh_profiler.frame_heat();
        let scene = &self.rendered_frame.scene;
        let custom_primitive_count = scene
            .custom_primitives
            .iter()
            .map(|(_, batch)| batch.count)
            .sum();
        self.frame_metrics = Some(crate::FrameMetrics {
            ...
            retained_dirty_fiber_count,
            refresh_heat,
            custom_primitive_count,
    ...
    /// Queue one instance of a custom (plugin) render primitive for this frame.
    ///
    /// `type_id` identifies the primitive type registered via
    /// [`App::register_render_primitive`]; `instance_bytes` are the plugin's raw
    /// `Pod` instance data (e.g. `solid_quad.as_bytes()`), and `bounds` is the
    /// element-space rect used for damage. Instances of the same type are batched
    /// and drawn in one GPU call after the built-in primitives. This is the public
    /// painting entry point for the multi-crate primitive architecture.
    pub fn paint_custom_primitive(
        &mut self,
        type_id: crate::PrimitiveTypeId,
        instance_bytes: &[u8],
        bounds: Bounds<Pixels>,
    ) {
        self.invalidator.debug_assert_paint();
        let scaled = bounds.scale(self.scale_factor());
        let prim_bounds = crate::PrimBounds::new(
            scaled.origin.x.0,
            scaled.origin.y.0,
            scaled.size.width.0,
            scaled.size.height.0,
        );
        self.next_frame
            .scene
            .custom_primitives
            .push(type_id, instance_bytes, prim_bounds);
    }
    ...
    fn current_view_id(&self) -> Option<EntityId> {
        self.rendered_entity_stack.last().copied()
    }

    pub(crate) fn record_retained_node_id(
        &mut self,
        global_id: &GlobalElementId,
    ) -> crate::retained::FiberId {
        let fiber_id = self
            .next_frame
            .retained_fiber_tree
            .reconcile_element(global_id, &self.rendered_frame.retained_fiber_tree);
        if let Some(view_id) = self.current_view_id() {
            self.next_frame
                .retained_node_ids_by_view
                .entry(view_id)
                .or_default()
                .push(global_id.clone());
        }
        fiber_id
    }

    pub(crate) fn retained_node_ids_for_view(&self, view_id: EntityId) -> Vec<GlobalElementId> {
        self.next_frame
            .retained_node_ids_by_view
            .get(&view_id)
            .cloned()
    ...
    /// Carry a reused view's fiber identities forward into the next frame. When a
    /// view's prepaint is replayed from cache its element subtree is not walked,
    /// so its fibers are carried forward in bulk from the previous frame (a cheap
    /// id-keyed copy — no element-path re-derivation or per-element allocation),
    /// keeping the retained tree a complete, cross-frame-stable mirror. The ids
    /// are re-recorded under the current view so the next reused frame can carry
    /// them forward again.
    pub(crate) fn update_retained_nodes(&mut self, global_ids: &[GlobalElementId]) {
        self.next_frame
            .retained_fiber_tree
            .carry_forward(global_ids, &self.rendered_frame.retained_fiber_tree);
        if let Some(view_id) = self.current_view_id() {
            self.next_frame
                .retained_node_ids_by_view
                .entry(view_id)
                .or_default()
                .extend(global_ids.iter().cloned());
        }
    }

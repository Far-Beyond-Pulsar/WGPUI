...
pub(crate) enum SurfaceContent {
    ...
    Wgpu(SurfaceId),
    /// A raw GPU texture passed directly by the caller.
    ///
    /// The texture is type-erased as `Arc<dyn Any>` to avoid a hard `wgpu`
    /// dependency in this crate; the WGPU renderer downcasts it to
    /// `Arc<wgpu::Texture>` at draw time.
    Texture {

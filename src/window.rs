...
impl Window {
    ...
    /// driver. This is [`InvalidationScope::Window`] with every axis set, which
    /// is the bluntest request the framework can express — it is correct for
    /// device loss or a scale factor change, and wasteful for everything else.
    /// It is not marked `#[deprecated]` only because it still has dozens of
    /// in-crate callers with nowhere better to go yet.

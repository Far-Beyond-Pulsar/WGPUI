...
impl Window {
    ...
    /// Paint a GPU texture directly into the scene for the next frame at the current z-index.
    ///
    /// `texture` must be an `Arc<wgpu::Texture>` created from the renderer's device.
    /// The renderer will downcast it at draw time; if the concrete type is not recognised
    /// the surface is silently skipped.
    ///
    /// This method should only be called as part of the paint phase of element drawing.
    pub fn paint_surface(
        ...
        self.next_frame.scene.insert_primitive(PaintSurface {
            ...
            bounds,
            content_mask,
            content: SurfaceContent::Texture { texture, size },
        });
    }

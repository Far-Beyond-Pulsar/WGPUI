...
impl Animation {
    ...
    pub fn new(duration: Duration) -> Self { Self { duration, oneshot: true, easing: Rc::new(linear) } }
    /// Repeat this animation forever.
    pub fn repeat(mut self) -> Self { self.oneshot = false; self }
    /// Set the easing function.
    pub fn with_easing(mut self, easing: impl Fn(f32) -> f32 + 'static) -> Self { self.easing = Rc::new(easing); self }
}

/// The result of sampling an animation sequence.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct AnimationSample { pub animation_index: usize, pub progress: f32, pub finished: bool }

/// A clock-bound sequence of animation definitions.
#[derive(Clone)]
pub struct AnimationTimeline { animations: Vec<Animation>, started: Instant }

impl AnimationTimeline {
    /// Start a non-empty sequence at `started`.
    pub fn new(animations: Vec<Animation>, started: Instant) -> Option<Self> { (!animations.is_empty()).then_some(Self { animations, started }) }
        ...
        for (index, animation) in self.animations.iter().enumerate() {
            ...
            if duration.is_zero() || elapsed >= duration {
                if !animation.oneshot && index == 0 && self.animations.len() == 1 {
                    let progress = if duration.is_zero() { 1.0 } else { (elapsed.as_secs_f64() / duration.as_secs_f64()).fract() as f32 };
                    return sample(animation, index, progress, false);
                }
                elapsed = elapsed.saturating_sub(duration);
                if index + 1 == self.animations.len() { return sample(animation, index, 1.0, true); }
            } else {
                return sample(animation, index, elapsed.as_secs_f64() as f32 / duration.as_secs_f64() as f32, false);
            }
        }
    ...
    pub fn request_next_frame(&self, now: Instant, scheduler: &mut AnimationScheduler) {
        if !self.sample_at(now).finished { scheduler.request_animation_frame(); }
    }
}

fn sample(animation: &Animation, animation_index: usize, progress: f32, finished: bool) -> AnimationSample {
    AnimationSample { animation_index, progress: (animation.easing)(progress.clamp(0.0, 1.0)).clamp(0.0, 1.0), finished }
}
...
impl<E: crate::div::IntoDescription> AnimationExt for E {
    fn with_animation(self, id: impl Into<ElementId>, animation: Animation, animator: impl Fn(Self, f32) -> Self + 'static) -> AnimationElement<Self> {
        self.with_animations(id, vec![animation], move |element, _, progress| animator(element, progress))
    }
    ...
    fn with_animations(self, id: impl Into<ElementId>, animations: Vec<Animation>, animator: impl Fn(Self, usize, f32) -> Self + 'static) -> AnimationElement<Self> {
        ...
        let timeline = match AnimationTimeline::new(animations, started) {
            ...
            None => AnimationTimeline { animations: vec![Animation::new(Duration::ZERO)], started },
        };
        AnimationElement { id: id.into(), element: self, timeline, animator: Box::new(animator) }
    }
}

/// An animated element whose output remains a normal `Description`.
pub struct AnimationElement<E> { id: ElementId, element: E, timeline: AnimationTimeline, animator: Box<dyn Fn(E, usize, f32) -> E> }
...
impl<E: crate::div::IntoDescription> AnimationElement<E> {
    ...
    pub fn describe_at(self, now: Instant) -> Description {
        ...
        let mut description = (self.animator)(self.element, sample.animation_index, sample.progress).into_description();
        if description.element_id().is_none() { description = description.id(self.id); }
        description
    }
    /// Request a future frame if the sequence is still active.
    pub fn request_next_frame(&self, now: Instant, scheduler: &mut AnimationScheduler) { self.timeline.request_next_frame(now, scheduler); }
}

impl<E: crate::div::IntoDescription> crate::div::IntoDescription for AnimationElement<E> {
    fn into_description(self) -> Description { self.describe_at(Instant::now()) }
}

/// Linear easing.
pub fn linear(value: f32) -> f32 { value }
/// Quadratic ease-in.
pub fn quadratic(value: f32) -> f32 { value * value }
/// Symmetric quadratic ease-in-out.
pub fn ease_in_out(value: f32) -> f32 { if value < 0.5 { 2.0 * value * value } else { 1.0 - (-2.0 * value + 2.0).powi(2) / 2.0 } }
/// Quintic ease-out.
pub fn ease_out_quint(value: f32) -> f32 { 1.0 - (1.0 - value).powi(5) }
...
mod tests {
    ...
    fn timeline_advances_and_clamps_a_chain() {
        ...
        let timeline = AnimationTimeline::new(vec![Animation::new(Duration::from_millis(100)), Animation::new(Duration::from_millis(100)).with_easing(|value| value * 2.0)], start).expect("definitions");
        assert_eq!(timeline.sample_at(start + Duration::from_millis(50)).progress, 0.5);
        assert_eq!(timeline.sample_at(start + Duration::from_millis(125)).animation_index, 1);
        assert_eq!(timeline.sample_at(start + Duration::from_millis(125)).progress, 0.5);
        assert!(timeline.sample_at(start + Duration::from_millis(250)).finished);
    }
    #[test]
    fn repeat_requests_another_frame() {
        let start = Instant::now();
        let timeline = AnimationTimeline::new(vec![Animation::new(Duration::from_millis(10)).repeat()], start).expect("definition");
        assert_eq!(timeline.sample_at(start + Duration::from_millis(25)).progress, 0.5);
    ...
    fn sampled_widget_output_is_an_ordinary_description_diff() {
        ...
        assert!(!first.key().expect("key").compare(second.key().expect("key")).is_empty());
    }
}

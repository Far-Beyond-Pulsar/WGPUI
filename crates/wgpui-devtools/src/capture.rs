...
impl fmt::Display for CaptureError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Serialization(message) => write!(formatter, "capture serialization failed: {message}"),
            Self::CaptureTooLarge { actual, maximum } => write!(
                formatter,
                "capture is {actual} bytes, exceeding the {maximum}-byte limit"
            ),
            Self::UnknownResource(id) => {
                write!(formatter, "resource {id:?} is not present in the frozen capture")
            }
            Self::InvalidReadbackRange => formatter.write_str("resource readback range is invalid"),
            Self::ReadbackTooLarge { requested, maximum } => write!(
                formatter,
                "resource readback requested {requested} bytes, exceeding the {maximum}-byte limit"
            ),
            Self::DuplicateResource => {
                formatter.write_str("a capture contains the same resource ID more than once")
            }
            Self::UnsupportedSchemaVersion(version) => {
                write!(formatter, "capture schema version {version} is not supported")
            }
            Self::InvalidFile => formatter.write_str("capture file has an invalid header"),
            Self::InvalidState => formatter.write_str("capture is not in a state that can be changed"),
        }
    }
}
...
const SERVICE_DISABLED: u8 = 0;
const SERVICE_ARMED: u8 = 1;
const SERVICE_COLLECTING: u8 = 2;
const SERVICE_STOP_REQUESTED: u8 = 3;
const SERVICE_FROZEN: u8 = 4;
...
impl CaptureController {
    ...
    pub fn state(&self) -> CaptureState {
        match self.state.load(Ordering::Acquire) {
            SERVICE_ARMED => CaptureState::Armed,
            SERVICE_COLLECTING => CaptureState::Collecting,
            SERVICE_STOP_REQUESTED => CaptureState::StopRequested,
            SERVICE_FROZEN => CaptureState::Frozen,
    ...
    pub fn arm(&self) -> Result<(), CaptureError> {
        self.state
            .compare_exchange(SERVICE_DISABLED, SERVICE_ARMED, Ordering::AcqRel, Ordering::Acquire)
            .map(|_| ())
    ...
    pub fn request_stop(&self) -> Result<(), CaptureError> {
        loop {
            ...
            let next = match state {
                SERVICE_ARMED | SERVICE_COLLECTING => SERVICE_STOP_REQUESTED,
    ...
    pub fn publish_frozen(&self, capture: FrozenCapture) -> Result<(), CaptureError> {
        ...
        *self
        ...
        self.state.store(SERVICE_FROZEN, Ordering::Release);
        Ok(())
    }
    ...
    pub fn reset(&self) {
        *self
        ...
        self.state.store(SERVICE_DISABLED, Ordering::Release);
    }
}
